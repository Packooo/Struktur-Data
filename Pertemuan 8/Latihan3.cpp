...
void sorting() {
	...
	for (int i = 0; i < n-1; i++) {
		cur = head;
		for (int j = i+1; j < n; j++) {
			if (cur->tahunTerbit > cur->next->tahunTerbit) {
				string tempJudul = cur->judul;
				string tempPengarang = cur->pengarang;
				int tempTahunTerbit = cur->tahunTerbit;


				cur->judul = cur->next->judul;
				cur->pengarang = cur->next->pengarang;
				cur->tahunTerbit = cur->next->tahunTerbit;

				cur->next->judul = tempJudul;
				cur->next->pengarang = tempPengarang;
				cur->next->tahunTerbit = tempTahunTerbit;
			}
			cur = cur->next;
		}
	}
...
int main() {
	...
	cout << "Data setelah di urutkan ke tahun terbit buku >> " <<endl;
